...
| 🏗️ **IR pipeline** | One rich intermediate file (`*.centurion.json`) chains every command: `convert → correct → translate → dub → build` |
| 🖥️ **REST API** | Centurion.Server runs the whole pipeline over HTTP |
...
| `transcribe` … `quality` | 🔬 Operator micro-commands — one pipeline stage at a time | `transcribe <INPUT_FILE>` … |
...
| [🖥️ Server](https://remembering-in-moskowien.github.io/Centurion/server) | Centurion.Server REST API |
| [🔬 Advanced](https://remembering-in-moskowien.github.io/Centurion/advanced) | Micro-commands, quality reports, self-update, localization |
...
| `Centurion.Core` | The engine: operators, pipeline executor, strategies, DI |
| `Centurion.Cli` | Spectre.Console CLI front-end |
| `Centurion.Server` | ASP.NET Core REST API |
