...
/// 供 <see cref="ScriptTimelineMapperOp"/>（台本打轴）与
/// <see cref="SubtitleTimelineCorrectorOp"/>（字幕校正）共同使用。
///
/// NW 打分（统一约定）：
///   精确匹配 → 3
///   模糊匹配（≥ 0.75）→ 2
...
/// </summary>
public abstract partial class TimelineAlignmentOpBase<TSelf> : PipelineOperatorBase<TSelf>
    where TSelf : TimelineAlignmentOpBase<TSelf>
{
    // ===== NW 打分 =====
    ...
    protected TimelineAlignmentOpBase(ILogger<TSelf> logger) : base(logger) { }
