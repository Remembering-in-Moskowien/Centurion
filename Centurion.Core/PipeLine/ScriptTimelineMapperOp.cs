...
///   4. 输出时间单调性二次校验。
///
/// 规模策略由基类 <see cref="TimelineAlignmentOpBase{TSelf}"/> 提供。
/// </summary>
public sealed class ScriptTimelineMapperOp : TimelineAlignmentOpBase<ScriptTimelineMapperOp>
{
    private readonly ILogger<ScriptTimelineMapperOp> _logger;

    public ScriptTimelineMapperOp(ILogger<ScriptTimelineMapperOp> logger) : base(logger)
    {
        _logger = logger;
    }
