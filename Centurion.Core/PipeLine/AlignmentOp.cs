...
/// </summary>
public class AlignmentOp : PipelineOperatorBase<AlignmentOp>, IHealthCheckableOperator
{
    ...
    public override async Task ExecuteAsync(SubtitleWorkflowContext context, CancellationToken cancellationToken)
    {
        ...
        var metadata = context.State.Extensions.TryGetValue("CorrectionMetadata", out var metadataValue)
            ? metadataValue as Dictionary<Sentence, Dictionary<string, object>>
            : null;
        var alignableSentences = sentences
            .Where(sentence => !IsKept(metadata, sentence))
            .ToList();
        var skippedCount = sentences.Count - alignableSentences.Count;
        if (skippedCount > 0)
            LogInfo($"Skipping forced alignment for {skippedCount} kept correction sentences.");
        if (alignableSentences.Count == 0)
        {
            context.State.AlignedSentences = sentences;
            context.State.IsAligned = true;
            OnProgress(100, "Alignment skipped for kept sentences");
            return;
        }
        ...
        var alignedSentences = await strategy.AlignAsync(alignableSentences, audioPath, cancellationToken);
        if (alignedSentences.Count != alignableSentences.Count)
        {
            var actualCount = alignedSentences?.Count ?? 0;
            var message = $"Alignment changed the sentence count from {alignableSentences.Count} to {actualCount}.";
            _logger.LogError(message);
            context.State.Errors.Add(message);
            throw new AlignmentException(message);
        }
        ...
        for (var index = 0; index < alignableSentences.Count; index++)
            alignedBySource[alignableSentences[index]] = alignedSentences[index];
        ...
        {
            foreach (var pair in alignedBySource)
            {
                if (metadata.TryGetValue(pair.Key, out var values))
                    metadata[pair.Value] = values;
            }
        }
        ...
        LogInfo($"Alignment finished. Processed {alignedSentences.Count} sentences; skipped {skippedCount}.");
    }
    ...
    private static bool IsKept(
        ...
        Sentence sentence)
    {
        return metadata is not null && metadata.TryGetValue(sentence, out var values) &&
               values.TryGetValue(Centurion.Core.Text.CorrectKeys.Action, out var action) &&
               Equals(action, "kept");
    }
